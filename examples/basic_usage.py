...
def main():
    ...
    try:
        ...
        print(f"  Agents: {', '.join([k for k, v in status['agents'].items() if v == 'initialized'])}")
        print(f"  Retrieval: {', '.join([k for k, v in status['retrieval'].items() if v == 'initialized'])}")
        ...
        if hasattr(runner, 'threat_intel_rm') and runner.threat_intel_rm:
            ...
            try:
                sample_data_path = Path(__file__).parent.parent / "data" / "threat_intel" / "sample_reports.csv"
